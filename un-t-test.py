...
# Button for two-sample t-test
ttest_btn = dbc.Button('Perform Two-Sample T-Test', id='ttest-btn', color='primary', style={'margin-top': '10px', 'margin-bottom': '10px'})
...
# Callback to perform two-sample t-test, display the result, and handle normality test
@app.callback(
...
def perform_ttest(ttest_clicks, selected_variables, uploaded_data):
    ...
    t_stat, p_value_ttest = ttest_ind(df_filtered[selected_variables[0]], df_filtered[selected_variables[1]])

    # Display t-test result with formatted p-value
    ttest_result_text = f"Two-Sample T-Test for {selected_variables[0]} and {selected_variables[1]}:\n\n"
